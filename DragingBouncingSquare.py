...
class trackingSquare():
    ...
    def spawning(self):
        pygame.draw.rect(WIN, self.color, pygame.Rect(self.position_x, self.position_y, self.size, self.size))

    def fading(self):
        for i in range(len(self.color)):
            if self.color[i] > 0:
                self.color[i] = int(self.color[i]) - 2
            else:
                pass
...
def main():
    ...
    while RUN:
        ...
        if len(trackingSquare.listOfAllObjects) > 80:
            del trackingSquare.listOfAllObjects[0]

        WIN.fill((0,0,0))
        trackingSquare(rectangle.x + rectangle.w //2, rectangle.y + rectangle.h //2)
        for object in trackingSquare.listOfAllObjects:
            trackingSquare.spawning(object)
            trackingSquare.fading(object) 
